...
public class FluentConstraint extends MultiBinaryConstraint {
	...
	public enum Type {
		MATCHES, DC, PRE, OPENS, CLOSES, BEFORE, UNARYAPPLIED, MOVEDURATION, AXIOM, RESOURCEUSAGE
	}
	...
	public FluentConstraint(Type t, Bounds ... bounds) {
		this.type = t;
		this.arrayBound = bounds;
	}


	@Override
	protected Constraint[] createInternalConstraints(Variable f, Variable t) {
		if (!(f instanceof Fluent) || !(t instanceof Fluent))
			return null;

		List<Constraint> retList = new ArrayList<Constraint>();

		if (this.type.equals(Type.MATCHES)) {
			AllenIntervalConstraint eq = new AllenIntervalConstraint(AllenIntervalConstraint.Type.Equals);
			...
			CompoundSymbolicValueConstraint ncon = new CompoundSymbolicValueConstraint(
					CompoundSymbolicValueConstraint.Type.MATCHES);
...
//				preAIC.setTo(((Fluent) f).getAllenInterval());
			...
			if (additionalConstraints != null && additionalConstraints.size() > 0) {
				...
				// AllenIntervalConstraint befCon = new
				// AllenIntervalConstraint(AllenIntervalConstraint.Type.Meets);
				// AllenIntervalConstraint befCon = new
				// AllenIntervalConstraint(AllenIntervalConstraint.Type.Overlaps, new Bounds(2L,
				// 6L));
				AllenIntervalConstraint befCon = new AllenIntervalConstraint(AllenIntervalConstraint.Type.Overlaps,
			...
			if (additionalConstraints != null && additionalConstraints.size() > 0) {
				...
				// TODO probably need other relations, too
				AllenIntervalConstraint oiCon = new AllenIntervalConstraint(AllenIntervalConstraint.Type.OverlappedBy,
			...
			AllenIntervalConstraint durationCon = new AllenIntervalConstraint(AllenIntervalConstraint.Type.Duration,
					this.bounds
			// new Bounds(10, 100)
			);
	...
	private CompoundSymbolicValueConstraint createSubmatches(Variable f, Variable t) {
		CompoundSymbolicValueConstraint ncon = new CompoundSymbolicValueConstraint(
				CompoundSymbolicValueConstraint.Type.SUBMATCHES, connections);
		CompoundSymbolicVariable nf = ((Fluent) f).getCompoundSymbolicVariable();
		ncon.setFrom(nf);
		CompoundSymbolicVariable nt = ((Fluent) t).getCompoundSymbolicVariable();
		ncon.setTo(nt);
	...
	public boolean isUsingResource(String resource) {
		if (resourceIndicator.getResourceName().equals(resource)) {
			...
			for (int i = 0; i < resourceRequirementPositions.length; i++) {
				...
				if (symbols.length != 1 || !symbols[0].equals(resourceIndicator.getResourceRequirements()[i])) {
					return false;
				}
			}
			return true;
