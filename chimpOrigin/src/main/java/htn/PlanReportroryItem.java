...
//			keyToFluentMap.put(HEAD_STRING, taskFluent);
			for (EffectTemplate et : effects) {
				...
				SignatureMap.put(et.getName(), et.getPrototype(groundSolver).getParameters());
				
			}
			
			for (FluentConstraint con : comb) {
				preKeyToFluentMap.put(constraintToPrecondition.get(con), (Fluent) con.getFrom());
			//	SignatureMap.put(con.getAxiom(), con.getScope()[0]);
...
//				for (RectangleConstraint RCCon : setRecAdditionalConstraints(taskFluent, preKeyToFluentMap, effKeyToVariableMap)) {
//					System.out.println(RCCon);
//				}
				...
				if (rectangularConstraintTemplates != null) {
					for (SpatialConstraintTemplate sc : rectangularConstraintTemplates) {

						if (sc.type3 != null) {
							Variable v1 = findSpatialVariable(sc.varKeys[0], taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);
							if (groundSolver.getConstraintSolvers()[2].getConstraints(v1, v1).length > 0) {
								Constraint[] cs = groundSolver.getConstraintSolvers()[2].getConstraints(v1, v1);
								for (Constraint a : cs) {
									String csName = a.getEdgeLabel().substring(0, a.getEdgeLabel().indexOf("["));
									if (csName.equals(sc.type3.toString()))
										groundSolver.getConstraintSolvers()[2].removeConstraint(a);
								}
								
							}


							Variable TBox1 = groundSolver.getConstraintSolvers()[2].getVariables(v1.getComponent().substring(0, v1.getComponent().length() - 1))[0];
							Constraint[] cs = groundSolver.getConstraintSolvers()[2].getConstraints(TBox1, v1);
							for (Constraint a : cs) {
								groundSolver.getConstraintSolvers()[2].removeConstraint(a);
							}

						} else // delete binary constraints between two variables
						if (sc.type3 == null && sc.type2 == null && sc.type1 == null&& sc.op1 == null) {

							Variable v1 = findSpatialVariable(sc.varKeys[0], taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);
							Variable v2 = findSpatialVariable(sc.varKeys[1], taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);

							Variable TBox1 = groundSolver.getConstraintSolvers()[2].getVariables(v1.getComponent().substring(0, v1.getComponent().length() - 1))[0];
							Variable TBox2 = groundSolver.getConstraintSolvers()[2].getVariables(v2.getComponent().substring(0, v2.getComponent().length() - 1))[0];
							
							//groundSolver.getConstraintSolvers()[2]
 

							if (groundSolver.getConstraintSolvers()[2].getConstraints(TBox1, v1).length > 0) {
								Constraint[] cs = groundSolver.getConstraintSolvers()[2].getConstraints(TBox1, v1);
								for (Constraint a : cs) {
									groundSolver.getConstraintSolvers()[2].removeConstraint(a);
								}
								Constraint[] cs1 = groundSolver.getConstraintSolvers()[2].getConstraints(TBox2, v2);
								for (Constraint a1 : cs1) {
									groundSolver.getConstraintSolvers()[2].removeConstraint(a1);
								}
								cs = groundSolver.getConstraintSolvers()[2].getConstraints(TBox2, TBox1);
								for (Constraint a : cs) {
									groundSolver.getConstraintSolvers()[2].removeConstraint(a);
								}


									// delete assertion : knife-->equals,equals -->knife1
									
									
									
									
//									if (sc.varKeys.length > 0) {
//										for (String i : sc.varKeys) {
//
//											Variable a1 = findSpatialVariable(i, taskFluent, groundSolver, SignatureMap,
//													preKeyToFluentMap);
//											String csAss = a1.getComponent().replaceAll("[^A-Za-z]", "");
//
//											Variable a2 = groundSolver.getConstraintSolvers()[2].getVariables(csAss)[0];
//											if (groundSolver.getConstraintSolvers()[2].getConstraints(a2,
//													a1).length > 0) {
//												Constraint binaryAssertionCs = groundSolver.getConstraintSolvers()[2]
//														.getConstraints(a2, a1)[0];
//												groundSolver.getConstraintSolvers()[2]
//														.removeConstraint(binaryAssertionCs);
//											}
//										}
//									}
//								}
						...
						} else if (sc.type1 != null) {
							Variable v1 = findSpatialVariable(sc.varKeys[0], taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);
							Variable v2;
							v2 = findSpatialVariable(sc.varKeys[1], taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);

							RectangleConstraint biCons = new RectangleConstraint(new AllenIntervalConstraint(sc.type1),
									new AllenIntervalConstraint(sc.type2));
							biCons.setFrom(v1);
							biCons.setTo(v2);
							System.out.println("added SC " + biCons.toString()+ " : "
									+ groundSolver.getConstraintSolvers()[2].addConstraint(biCons));
							cn.addConstraint(biCons);
						} else {
							String key1 = sc.varKeys[0];
							String key2 = sc.varKeys[1];
							Variable obj1 = findSpatialVariable(key1, taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);
							

							Variable obj2 = findSpatialVariable(key2, taskFluent, groundSolver, SignatureMap,
									preKeyToFluentMap);

							if (obj1 != null && obj2 != null) {

								// get object to shift
								Constraint[] constraintsToRemove = groundSolver.getConstraintSolvers()[2]
										.getConstraints(obj1, obj1);
								Constraint[] Unaryconstraints = groundSolver.getConstraintSolvers()[2]
										.getConstraints(obj2, obj2);
								UnaryRectangleConstraint toRemove = null, AtObj2 = null;

								// remove existing conflict constraints if it's exist
								for (Constraint a : constraintsToRemove) {
									UnaryRectangleConstraint b = (UnaryRectangleConstraint) a;
									if (b.getType().equals(UnaryRectangleConstraint.Type.At)) {
										toRemove = b;
										groundSolver.getConstraintSolvers()[2].removeConstraint(toRemove);
									}
								}

								for (Constraint a : Unaryconstraints) {
									if (((UnaryRectangleConstraint)a).getType().equals(UnaryRectangleConstraint.Type.Size)) {
										AtObj2= (UnaryRectangleConstraint) a;
									}
								}
								// add new shifted object position
								if(toRemove!=null) {
								UnaryRectangleConstraint toShift = (UnaryRectangleConstraint) toRemove.clone();

								toShift.setFrom(obj1);
								toShift.setTo(obj1);
								// X-perspective
								toShift.getBounds()[0].max += AtObj2.getBounds()[0].max + sc.cste;
								toShift.getBounds()[0].min += AtObj2.getBounds()[0].min + sc.cste;
								toShift.getBounds()[1].max += AtObj2.getBounds()[1].max + sc.cste;
								toShift.getBounds()[1].min += AtObj2.getBounds()[1].min + sc.cste;

								System.out.println(toShift.toString());
//								System.out.println("shifted obj( " + key1 + " )"
//										+ groundSolver.getConstraintSolvers()[2].addConstraint(toShift));
								}
							}
						}
					}
...
//					}
	...
	private Variable findSpatialVariable(String id, Fluent taskFluent, FluentNetworkSolver groundSolver,
			Map<String, Object[]> signatureMap, Map<String, Fluent> preKeyToFluentMap) {
		Variable var = null;
		String[] signature = taskFluent.getSpatialVariable();

		String[] arguments = taskFluent.getCompoundSymbolicVariable().getArgs();

		Map<String, String> varSpacialMap = new HashMap<String, String>(arguments.length);
		for (int i = 0; i < arguments.length; i++) {
			varSpacialMap.put(signature[i], arguments[i]);
		}
		String VariableName = varSpacialMap.get(id);
		
		Variable[] vv = groundSolver.getConstraintSolvers()[0].getVariables();


		if(groundSolver.getConstraintSolvers()[2].getVariables(VariableName).length>0)
		var = groundSolver.getConstraintSolvers()[2].getVariables(VariableName)[0];
		else if (groundSolver.getConstraintSolvers()[2].getVariables(id.substring(1)).length>0)
			var = groundSolver.getConstraintSolvers()[2].getVariables(id.substring(1))[0];
		else if (VariableName ==null) {//get isColission variables
			for (int i = 0; i < vv.length; i++) {
				String constraintName = vv[i].toString().substring(0, vv[i].toString().indexOf("("));
				if(constraintName.equals("isCollision")) {
					String part1 =vv[i].toString().split(" ")[0];
					String obj1 = part1.substring(part1.indexOf("(")+1);

					String obj2 =vv[i].toString().split(" ")[1];
					if(varSpacialMap.get(obj1)==null) 
						var = groundSolver.getConstraintSolvers()[2].getVariables(obj1)[0];
					else
						var = groundSolver.getConstraintSolvers()[2].getVariables(obj2)[0];
					break;
				}								
		...
		else{//create spatial variable if it's exist
			 var = (RectangularRegion) groundSolver.getConstraintSolvers()[2].createVariable();
			((RectangularRegion) var).setName(VariableName);		
