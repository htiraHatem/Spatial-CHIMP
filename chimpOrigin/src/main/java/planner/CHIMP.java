...
public class CHIMP {
	...
	private SpatialFrame spatialFrame = null;
	...
	private CHIMP(CHIMPBuilder builder) {
		...
		if (builder.problem.createState(fluentSolver, builder.domain))
			System.out.println("rectangular constraint propagated!");
		else
			System.out.println("rectangular constraint: propagation problems(conflict)!");
	...
	/**
	 * Draws rectangular region final values after applying constraints.
	 * draw also spatial constraint network
	 */
	public void drawRectangularRegion() {

		Graph<Variable, Constraint> g = new DirectedSparseMultigraph<Variable, Constraint>();
		for (Constraint con : fluentSolver.getConstraintSolvers()[2].getConstraintNetwork().getConstraints()) {
			if (con instanceof RectangleConstraint) {
				RectangleConstraint fc = (RectangleConstraint) con;
					g.addEdge( fc,fc.getFrom(), fc.getTo());
			}
			if (con instanceof UnaryRectangleConstraint) {
				UnaryRectangleConstraint fc = (UnaryRectangleConstraint) con;
					g.addEdge( fc,fc.getFrom(), fc.getTo());
			}
		}

		try {
			this.spatialFrame = SpatialFrame.drawRec(g, 100);
		} catch (Exception e) {
	...
	public void printFullPlan() {
		...
		Variable[] vars= fluentSolver.getConstraintSolvers()[2].getVariables();
		
		for (int i = 0; i < vars.length; i++)
			try {
			if (((RectangularRegion) vars[i]).getName().isEmpty())
				fluentSolver.getConstraintSolvers()[2].removeVariable(vars[i]);
			}catch(IllegalVariableRemoval e) {}
	...
	public void printStats(PrintStream ps) {
		if (foundPlan) {
			...
			for (Variable var : fluentSolver.getConstraintSolvers()[2].getVariables()) {
				if (!((RectangularRegion) var).getName().isEmpty())
					sCount++;
			}
			...
			ps.println("#Spatial Fluent: " + sCount);
			int sum= binaryCCount + unaryCCount;
			ps.println("#Spatial Constraints: " + sum);
			ps.println("#Spatial unaryConstraints: " + unaryCCount);
			ps.println("#Spatial binaryConstraints: " + binaryCCount);
		} else {
