...
spatial_constraint_def : spatial_constraint1_def | spatial_constraint2_def;

spatial_constraint1_def : '(SC' '(' binary_temporal_constraint_type ',' binary_temporal_constraint_type ')' '(' VAR_NAME ',' VAR_NAME ')' ')';

spatial_constraint2_def : '(SC'  VAR_NAME integer_operator1 VAR_NAME integer_operator2 var_or_int ')';

delete_spatial_constraint_def : delete_spatial_constraint1_def | delete_spatial_constraint2_def;

delete_spatial_constraint1_def : '(DelSC' '(' unary_spatial_constraint_type ')' '(' VAR_NAME ')' ')';

delete_spatial_constraint2_def : '(DelSC' '(' VAR_NAME ',' VAR_NAME ')' ')';
...
binary_temporal_constraint_type : 'Before' | 'Meets' | 'Overlaps' | 'FinishedBy' | 'Contains' | 'StartedBy'| 'Equals'
                           ...
                           | 'EndsDuring' | 'EndsOrEndedBy' | 'StartsOrStartedBy' | 'NotBeforeAndNotAfter' | 'Duration';

unary_spatial_constraint_type : 'At' | 'Size';
